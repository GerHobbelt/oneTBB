...
void* MapMemory(size_t bytes, PageType pageType)
{
    ...
    //return VirtualAlloc(nullptr, bytes, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE); // Arma 3 CMA - Add large pages support
    return VirtualAlloc(nullptr, bytes, MEM_RESERVE | MEM_COMMIT | (REGULAR == pageType ? NULL : MEM_LARGE_PAGES), PAGE_READWRITE); // Arma 3 CMA - Add large pages support
}
